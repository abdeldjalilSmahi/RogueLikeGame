...
public class Amicalstategie implements Strategy{
    ...
    public Amicalstategie() {
        this.armeContacts = Arrays.asList(ArmeContactType.values()).stream().map(e -> new ArmeContact(e)).collect(Collectors.toList());
        this.armeDistances = Arrays.asList(ArmeDistanceType.values()).stream().map(e -> new ArmeDistance(e)).collect(Collectors.toList());
        this.armures = Arrays.asList(ArmureType.values()).stream().map(e -> new Armure(e)).collect(Collectors.toList());
    }
