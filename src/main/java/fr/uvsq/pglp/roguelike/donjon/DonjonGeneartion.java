        ...
        for (int i = 0; i < profondeur; i++) {
            for (int j = 0; j < largeur; j++) {
                if (i == 0 || i == profondeur - 1 || j == 0 || j == largeur - 1) {
                    ...
                    ComponentDonjon componentDonjon = new ComponentDonjon(point2D, Color.CYAN, DonjonComponentType.OBSTACLE);
                    map[i][j] = componentDonjon ;
                    this.addToHashMap(point2D, componentDonjon );
                } else {
                    Point2D point2D = new Point2D.Double(i, j);
                    ComponentDonjon componentDonjon =  new ComponentDonjon(new Point2D.Double(i, j), Color.CYAN, DonjonComponentType.SOL);
                    map[i][j] =  componentDonjon ;
                    this.addToHashMap(point2D, componentDonjon );
                }
                for (k = 1; k < nbrfloors; k++) {
                    Point2D point2D = new Point2D.Double((roomSize + 1) * k, j) ;
                    ComponentDonjon componentDonjon = new ComponentDonjon(new Point2D.Double(i, j), Color.CYAN, DonjonComponentType.OBSTACLE);
            ...
            for (k = 1; k < nbRooms; k++) {
                ...
                ComponentDonjon componentDonjon = new ComponentDonjon(new Point2D.Double(i, (roomSize + 1) * k), Color.CYAN, DonjonComponentType.OBSTACLE);
    ...
    private void createportealeatoire() {
        for(int level=1;level<nbrfloors;level++){
            for (k = 1; k <= nbRooms; k++) {
                ...
                ComponentDonjon componentDonjon = new ComponentDonjon(new Point2D.Double((roomSize + 1) * level, random), Color.CYAN, DonjonComponentType.PORTE);
        ...
        for(int room=1;room<nbRooms;room++){
            for (k = 1; k <= nbrfloors; k++) {
                ...
                ComponentDonjon componentDonjon = new ComponentDonjon(new Point2D.Double(random, (roomSize + 1) * room), Color.CYAN, DonjonComponentType.PORTE);
        ...
        for (RoomDonjon roomDonjon : roomsDonjon){
            ...
            for(int k=0;k<nbrofdange;k++) {
                ...
                do {
                    ...
                    int y = rand.nextInt((int) (roomDonjon.getHautDroit().getY() - roomDonjon.getHautGauche().getY()) + 1)
